...
try:
    ...
    if sys.platform.startswith('linux'):
        sys.stderr.write('`wget --no-check-certificate --header="Host: gtor.googlecode.com" http://www.google.cn/files/gevent-1.0dev-linux-x86.egg`\n')
    elif sys.platform == 'darwin' and platform.processor() == 'i386':
        sys.stderr.write('`wget --no-check-certificate --header="Host: gtor.googlecode.com" http://www.google.cn/files/gevent-1.0dev-macosx-intel.egg`\n')
    elif os.name == 'nt':
    ...
    def import_ca(certfile):
        ...
        if sys.platform.startswith('win'):
            ...
            if platform_distname == 'Ubuntu':
                ...
                logging.warning('please install *libnss3-tools* package to import Gtor root ca')
        return 0
...
class HTTP(object):
    ...
    def __init__(self, max_window=4, max_timeout=32, max_retry=1, proxy='', ssl_validate=False):
    ...
    def parse_payload(self, rfile, bufsize=1048576):
        line = rfile.readline(bufsize)
        print 'parse_payload'
        print line
        if not line:
        ...
        for i in xrange(self.max_retry):
            ...
            try:
                ...
                if sock:
                    ...
                    self._request(ssl_sock or sock, method, path, self.protocol_version, headers, payload, bufsize=bufsize, crlf=crlf, return_sock=return_sock)
            except Exception as e:
...
def gae_urlfetch(method, url, headers, payload, fetchserver, **kwargs):
    ...
    if payload:
        ...
        logging.info('******gae_urlfetch*******tor-packet-len:%d****************', len(payload))
    ...
    http.request('POST', fetchserver, gae_payload, {'Content-Length': len(gae_payload)}, crlf=need_crlf)
...
class GAEProxyHandler(object):
    ...
    def handle_method_urlfetch(self):
        ...
        try:
            ...
            for i in xrange(common.FETCHMAX_LOCAL):
                try:
                    ...
                    if common.GAE_VALIDATE:
                        kwargs['validate'] = 1
                    self.urlfetch(self.method, self.path, self.headers, payload, common.GAE_FETCHSERVER, **kwargs)
                    break
                except Exception as e:
                    errors.append(e)
            ...
            try:
            	gaesock,gaeaddr = g_gaesock.accept()
            ...
            try:
            	print 'from gae connect:',gaesock.getpeername()
            	fdset = [self.sock, gaesock]
            	while True:
            		r,w,e = select.select(fdset, [], [])
            		if self.sock in r:
            			requestdata = self.sock.recv(4096)
            			if len(requestdata) <= 0:
            				continue
            			result = gaesock.sendall(requestdata)
            			logging.info('send to gae forward bridge')
            			
            		if gaesock in r:
            			responsedata = gaesock.recv(4096)
            			if len(responsedata) <=0:
            				continue
            			if responsedata.find('end-gtor') > 0:
            				logging.info('socket connections is restarted,wait for a moment*...')
            				break
            			result = self.sock.sendall(responsedata)
            			logging.info('send to tor client') 

            except:
            	logging.info('Socket connections may blocked, wait for a moment ....')
            finally:
            	self.sock.close()
            	gaesock.close()		
    ...
    def handle_connect_urlfetch(self):
        """deploy fake cert to client 200 ok"""
        host, _, port = self.path.rpartition(':')
        port = int(port)
        certfile = CertUtil.get_cert(host)
        logging.info('send tor client HTTP/1.1 200 OK')
...
def pre_start():
    ...
    if ctypes and os.name == 'nt':
        ctypes.windll.kernel32.SetConsoleTitleW(u'Gtor v%s' % __version__)
        if not common.LOVE_TIMESTAMP.strip():
            sys.stdout.write('Double click addto-startup.vbs could add gtor to autorun programs. :)\n')
        ...
        if softwares:
            ...
            if softwares:
                error = u'某些安全软件(如 %s)可能和本软件存在兼容性问题。\n如有此现象建议暂时退出此安全软件来继续运行Gtor' % ','.join(softwares)
                ctypes.windll.user32.MessageBoxW(None, error, u'gtor 建议', 0)
                #sys.exit(0)
    if common.GAE_APPIDS[0] == 'Gtor' and not common.CRLF_ENABLE:
...
g_gaesock = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
g_gaesock.bind(('',int(sockport)))
g_gaesock.listen(int(SOCKNUM))
